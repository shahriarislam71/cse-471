...
const DoctorRequest = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const userRequests = volunteersRes.data
          .filter(assignment => 
            assignment.doctors?.some(v => v.email === currentUserEmail)
          )
          .map(assignment => {
            const calamity = ongoingCalamities.find(c => c._id === assignment.calamityId);
            const volunteerInfo = assignment.doctors?.find(v => v.email === currentUserEmail);
  ...
  const handleResponse = async (id, action) => {
    try {
      ...
      await axios.patch(`https://health-and-sanitation-backend.vercel.app/doctors/${id}/respond`, { 
        action,
